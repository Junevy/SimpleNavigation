...
namespace SimpleNavigation.Services
{
    public class NavigationService : INavigationService
    {
        ...
        public void RegisterRoute<TPage>(DialogOptions? options = null) where TPage : Page, new()
        {
            RegisterRoute(nameof(TPage), () => new TPage(), options);
        }


        public void RegisterRoute<TPage>(string route, DialogOptions? options = null) where TPage : Page, new()
        {
            RegisterRoute<TPage>(route, () => new TPage(), options);
        }


        public void RegisterRoute<TPage>(string route, Func<Page> factory, DialogOptions? options = null) where TPage : Page
        {
            RegisterRoute(route, factory, options, typeof(TPage));
        }


        public void RegisterRoute<TPage>(Func<Page> factory, DialogOptions? options = null)
        {
            RegisterRoute(typeof(TPage).ToString(), factory, options, typeof(TPage));
        }


        public void RegisterRoute(string route, Func<Page> factory, DialogOptions? options = null, Type? pageType = null)
        {
        ...
        public void Navigate<TPage>(string region, DialogParameters? parameters = null)
        {
            Navigate(region, typeof(TPage).ToString(), parameters);
        }

        public void Navigate(string region, string route, DialogParameters? parameters = null)
        {
            ...
            if (targetPage.Options.AllowMulti == DialogOptions.DialogMode.Singleton)
            {
            ...
            // 清除导航历史
            if (targetPage.Options.History == DialogOptions.KeepHistory.Never)
            {
        ...
        private void InvokeNavigating(object? oldPage, DialogParameters parameters)
        {
        ...
        private void InvokeNavigated(Page pageInstance, DialogParameters parameters)
        {
